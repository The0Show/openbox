	...
	export class ImportPrompt implements Prompt {
		...
		public readonly container: HTMLDivElement = div({class: "prompt noSelection", style: "width: 300px;"},
			...
			p({style: "text-align: left; margin: 0.5em 0;"},
				"OpenBox songs can be exported and re-imported as .json files. You could also use other means to make .json files for OpenBox as long as they follow the same structure.",
			),
			p({style: "text-align: left; margin: 0.5em 0;"},
				"OpenBox can also (crudely) import .mid files. There are many tools available for creating .mid files. Shorter and simpler songs are more likely to work well.",
			),
			this._fileInput,
			this._cancelButton,
		);
